...
def cnn_model_fn(features, labels, mode):
	...
	conv1_1 = tf.layers.conv1d(inputs=x_image1, filters=ns.C1_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)

	pool1_1 = tf.layers.max_pooling1d(inputs=conv1_1, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# second conv layer

	conv2_1 = tf.layers.conv1d(inputs=pool1_1, filters=ns.C2_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)

	pool2_1 = tf.layers.max_pooling1d(inputs=conv2_1, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# third conv layer

	conv3_1 = tf.layers.conv1d(inputs=pool2_1, filters=ns.C3_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)

	pool3_1 = tf.layers.max_pooling1d(inputs=conv3_1, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# first conv layer
	conv1_2 = tf.layers.conv1d(inputs=x_image2, filters=ns.C1_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)

	pool1_2 = tf.layers.max_pooling1d(inputs=conv1_2, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# second conv layer

	conv2_2 = tf.layers.conv1d(inputs=pool1_2, filters=ns.C2_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)

	pool2_2 = tf.layers.max_pooling1d(inputs=conv2_2, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# third conv layer

	conv3_2 = tf.layers.conv1d(inputs=pool2_2, filters=ns.C3_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)
...
def main(argv):
	...
	ns.load_settings_late(dataset, '1d')
