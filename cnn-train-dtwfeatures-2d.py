...
def cnn_model_fn(features, labels, mode):
	...
	conv1 = tf.layers.conv2d(inputs=x_image, filters=ns.C1_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu, data_format='channels_last')

	pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# second conv layer

	conv2 = tf.layers.conv2d(inputs=pool1, filters=ns.C2_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)

	pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# third conv layer

	conv3 = tf.layers.conv2d(inputs=pool2, filters=ns.C3_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)
...
def main(argv):
	...
	ns.load_settings_dtwfeatures(dataset, '2d')

	run_name = "dtw-features-2d-50x50-fc1024-lr{0}-adam-{1}conv-{2}".format(ns.LEARNING_RATE, conv_shape, dataset) #+"-"+nowtime
