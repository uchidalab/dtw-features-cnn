...
if __name__ == "__main__":
    ...
    for version in ["1a", "1b", "1c"]:
        ...
        # load data
        data_sets = ds.read_data_sets(ns.TRAINING_FILE, ns.TEST_FILE, ns.IMAGE_SHAPE, train_label=ns.TRAINING_LABEL, test_label=ns.TEST_LABEL, validation_ratio=0.0, pickle=False, boring=False)
        ...
        train_data = data_sets.train.images.reshape((-1,50,2))
        train_labels = data_sets.train.labels

        train_number = np.shape(train_labels)[0] # 11700

        test_data = data_sets.test.images.reshape((-1,50,2))
        test_labels = data_sets.test.labels
        test_number = np.shape(test_labels)[0] #1300
        ...
        # start generation
        test_dtw_fileloc = "data\\data_features-50-test-data-"+version+".txt"
        test_combined_fileloc = "data\\data_features-50-plus-raw-test-data-"+version+".txt"
        train_dtw_fileloc = "data\\data_features-50-train-data-"+version+".txt"
        train_combined_fileloc = "data\\data_features-50-plus-raw-train-data-"+version+".txt"
