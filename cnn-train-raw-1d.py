...
def cnn_model_fn(features, labels, mode):
	...
	conv1 = tf.layers.conv1d(inputs=x_image, filters=ns.C1_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)

	pool1 = tf.layers.max_pooling1d(inputs=conv1, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# second conv layer

	conv2 = tf.layers.conv1d(inputs=pool1, filters=ns.C2_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)

	pool2 = tf.layers.max_pooling1d(inputs=conv2, pool_size=ns.MPOOL_SHAPE, strides=ns.MPOOL_SHAPE, padding='SAME')

	# third conv layer

	conv3 = tf.layers.conv1d(inputs=pool2, filters=ns.C3_LAYER_SIZE, kernel_size=conv_shape, padding='SAME', activation=tf.nn.relu)
	...
	if mode == tf.estimator.ModeKeys.TRAIN:
		#optimizer = tf.train.GradientDescentOptimizer(learning_rate=ns.LEARNING_RATE)
...
def main(argv):
	...
	ns.load_settings_raw(dataset, '1d')

	run_name = "raw-fc1024-lr{0}-adam-{1}conv-{2}".format(ns.LEARNING_RATE, conv_shape, dataset) #+"-"+nowtime

	print(run_name)

	data_sets = input_data.read_data_sets(ns.TRAINING_FILE, ns.TEST_FILE, ns.IMAGE_SHAPE, train_label=ns.TRAINING_LABEL, test_label=ns.TEST_LABEL, validation_ratio=0.0, pickle=False, boring=False)
